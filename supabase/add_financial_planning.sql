create table if not exists public.monthly_budgets (
  ...
  month date not null, category text not null, limit_cents integer not null check (limit_cents > 0), mode text not null default 'balanced' check (mode in ('comfortable','balanced','saving','custom')),
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique(user_id, month, category)
);
create table if not exists public.savings_goals (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade,
  name text not null, target_cents integer not null check (target_cents > 0), saved_cents integer not null default 0 check (saved_cents >= 0), target_date date,
  account_id uuid references public.accounts(id) on delete set null, color text not null default '#b49cff', active boolean not null default true, created_at timestamptz not null default now()
);
create table if not exists public.custom_categories (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade,
  name text not null, icon text not null default '•', color text not null default '#b49cff', kind text not null default 'expense' check(kind in ('expense','income')),
  created_at timestamptz not null default now(), unique(user_id, name, kind)
);
create table if not exists public.category_rules (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade,
  match_text text not null, category text not null, created_at timestamptz not null default now(), unique(user_id, match_text)
);
create table if not exists public.cash_reconciliations (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade,
  account_id uuid not null references public.accounts(id) on delete cascade, expected_cents integer not null, counted_cents integer not null,
  difference_cents integer not null, reconciled_at timestamptz not null default now()
);
create table if not exists public.financial_audit_log (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade,
  action text not null, entity_type text not null, entity_id uuid, detail jsonb not null default '{}'::jsonb, created_at timestamptz not null default now()
);
create table if not exists public.financial_plans (
  user_id uuid primary key references auth.users(id) on delete cascade,
  strategy text not null default 'prioritize_savings' check(strategy in ('prioritize_savings','balanced','flexible','custom')),
  monthly_income_cents integer not null check(monthly_income_cents >= 0), personal_allowance_cents integer not null default 0 check(personal_allowance_cents >= 0),
  surplus_destination text not null default 'savings' check(surplus_destination in ('savings','goals','available','split')),
  savings_account_id uuid references public.accounts(id) on delete set null, include_credit_estimate boolean not null default true,
  savings_role_confirmed boolean not null default false, updated_at timestamptz not null default now()
);
create table if not exists public.account_roles (
  user_id uuid not null references auth.users(id) on delete cascade, account_id uuid primary key references public.accounts(id) on delete cascade,
  role text not null check(role in ('available','savings','emergency','goal')), confirmed_at timestamptz not null default now()
);
