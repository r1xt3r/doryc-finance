...
const exportTables = ['accounts', 'transactions', 'recurring_payments', 'shared_payment_contributions', 'credit_cards', 'credit_card_purchases', 'credit_card_payments', 'personal_loans', 'personal_loan_payments', 'bank_loans', 'bank_loan_payments', 'monthly_budgets', 'savings_goals', 'custom_categories', 'category_rules', 'cash_reconciliations', 'financial_audit_log', 'financial_plans', 'account_roles', 'user_preferences'] as const;
const optionalTables = new Set<string>(['bank_loan_payments', 'shared_payment_contributions', 'monthly_budgets', 'savings_goals', 'custom_categories', 'category_rules', 'cash_reconciliations', 'financial_audit_log', 'financial_plans', 'account_roles']);
...
export async function GET(request: Request) {
  ...
  const results = await Promise.all(exportTables.map(async (table) => {
    ...
    if (error && !optionalTables.has(table)) throw error;
