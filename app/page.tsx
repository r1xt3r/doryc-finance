...
import { belongsToPlanningWindow, planningMonthKey } from '../lib/planningCycle';
...
const accountInitial = (name: string) => {
  ...
  return (words.at(-1)?.match(/\p{L}/u)?.[0] || name.match(/\p{L}/u)?.[0] || 'A').toUpperCase();
};
const bankIdentity = (bank: string) => {
  const normalized = bank.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  if (normalized.includes('pichincha')) return { key: 'pichincha', mark: 'P' };
  if (normalized.includes('produbanco') || normalized.includes('promerica')) return { key: 'produbanco', mark: 'Pr' };
  if (normalized.includes('pacifico')) return { key: 'pacifico', mark: 'Pa' };
  if (normalized.includes('guayaquil')) return { key: 'guayaquil', mark: 'G' };
  return { key: 'other', mark: bank.trim().slice(0, 1).toUpperCase() || 'B' };
};
function BankMark({ bank }: { bank: string }) {
  const identity = bankIdentity(bank);
  if (identity.key === 'pichincha') return <span className="bank-logo" aria-hidden="true"><svg viewBox="0 0 32 32"><path d="M7 5h18v22H7z" rx="4"/><path className="cut" d="M17 5h8v8h-5l-3-3z"/></svg></span>;
  if (identity.key === 'produbanco') return <span className="bank-logo" aria-hidden="true"><svg viewBox="0 0 32 32"><path d="m16 3 3.2 8.1L28 8l-4.8 7.5 7.8 4.3-8.9.4.8 8.8-5.4-7-6.5 6 2.3-8.5-8.7-1.8 8.6-2.1-6.3-6.2 9 .7z"/></svg></span>;
  if (identity.key === 'pacifico') return <span className="bank-logo" aria-hidden="true"><svg viewBox="0 0 32 32"><path d="M6 7h11.5a8.5 8.5 0 0 1 0 17H13v5H6zm7 6v5h4.5a2.5 2.5 0 0 0 0-5z"/></svg></span>;
  if (identity.key === 'guayaquil') return <span className="bank-logo" aria-hidden="true"><svg viewBox="0 0 32 32"><path d="M25.7 10.3A11 11 0 1 0 27 20h-9v-6h13v3a15 15 0 1 1-4.7-10.9z"/></svg></span>;
  return <span className="bank-logo bank-logo-fallback" aria-hidden="true">{identity.mark}</span>;
}
...
export default function Home() {
  ...
  const localizedError = (message: string) => {
    if (/including the transfer fee/i.test(message)) return tr('This account does not have enough funds to cover the transfer and its fee.', 'Esta cuenta no tiene fondos suficientes para cubrir la transferencia y su comisión.');
    if (/insufficient funds/i.test(message)) return tr('The selected account does not have enough funds for this movement.', 'La cuenta seleccionada no tiene fondos suficientes para realizar este movimiento.');
    if (/insufficient available credit/i.test(message)) return tr('This card does not have enough available credit.', 'Esta tarjeta no tiene suficiente crédito disponible.');
    return message;
  };
  ...
  const [accountPreset, setAccountPreset] = useState<'bank' | 'cash'>('bank');
  ...
  const [sharedPaymentEnabled, setSharedPaymentEnabled] = useState(false);
  const [cardModal, setCardModal] = useState<'account' | 'card' | 'purchase' | 'statement' | 'loan' | 'cardPayment' | 'loanPayment' | 'loanIncrease' | 'bankLoan' | 'sharedContribution' | null>(null);
  const [selectedCardId, setSelectedCardId] = useState('');
  const [selectedSharedParticipant, setSelectedSharedParticipant] = useState('');
  ...
  const availableAccounts = data.accounts.filter((account) => !isSavingsAccount(account));
  ...
  const accountsByBank = (() => {
    ...
    for (const account of data.accounts) {
      if (normalizeAccountType(account.accountType) === 'cash') continue;
      grouped.set(account.bank, [...(grouped.get(account.bank) || []), account]);
    }
  ...
  const sharedPayments = recurringExpenses.filter((item) => item.sharedMembers?.length > 0 && belongsToPlanningWindow(item.next_due_date, activePlanningMonth));
  const currentCycleMonth = `${activePlanningMonth}-01`;
  const openPersonalLoans = data.personalLoans.filter((loan) => loan.status !== 'settled' && loan.paid < loan.amount);
  ...
  const allFundingPayments = [
    ...
    ...planningBankLoans.filter((loan) => loan.outstandingBalance > 0 && loan.installment > 0 && bankLoanAccountId(loan)).map((loan) => ({ id: `bank-loan-${loan.id}`, sourceId: loan.id, name: loan.name, amount: Math.min(loan.installment, loan.outstandingBalance), next_due_date: loan.nextDueDate, pay_from_account_id: bankLoanAccountId(loan)!, payment_method: 'Automatic Debit', kind: 'bankLoan' as const })),
    ...planningCardSummaries.filter(({ card, estimatedPayment }) => estimatedPayment > 0 && card.payFromAccountId).map(({ card, estimatedPayment, nextPaymentDate }) => ({ id: `credit-card-${card.id}`, sourceId: card.id, name: `${card.name} payment`, amount: estimatedPayment, next_due_date: nextPaymentDate, pay_from_account_id: card.payFromAccountId!, payment_method: 'Bank Transfer', kind: 'creditCard' as const })),
  ];
  const currentMonthKey = today.slice(0, 7);
  const activeFundingMonth = activePlanningMonth;
  ...
  const financialTimeline = [
    ...
    ...planningBankLoans.filter((loan) => loan.outstandingBalance > 0).map((loan) => ({ id: `loan-${loan.id}`, name: loan.name, date: loan.nextDueDate, amount: Math.min(loan.installment, loan.outstandingBalance), kind: 'Automatic Debit', direction: 'expense' as const })),
    ...planningCardSummaries.filter(({ estimatedPayment }) => estimatedPayment > 0).map(({ card, estimatedPayment, nextPaymentDate }) => ({ id: `card-${card.id}`, name: `${card.name} payment`, date: nextPaymentDate, amount: estimatedPayment, kind: 'Bank Transfer', direction: 'expense' as const })),
  ].sort((a, b) => a.date.localeCompare(b.date)).slice(0, 6);
  const fundingMonth = fundingMonthLabel(activeFundingMonth);
  const savingsAccounts = data.accounts.filter(isSavingsAccount);
  const cashAccounts = data.accounts.filter((account) => normalizeAccountType(account.accountType) === 'cash');
  ...
  const setupMissions = [
    ...
    { title: tr('Expected income', 'Ingreso esperado'), detail: tr('Schedule your salary or regular monthly income.', 'Programa tu sueldo o ingreso mensual habitual.'), done: expectedIncome.length > 0, action: () => { setRecurringFlow('income'); setSharedPaymentEnabled(false); setActiveAction('Recurring'); } },
    { title: tr('Monthly payments', 'Pagos mensuales'), detail: tr('Register at least one recurring commitment.', 'Registra al menos un compromiso recurrente.'), done: recurringExpenses.length > 0, action: () => { setRecurringFlow('expense'); setSharedPaymentEnabled(false); setActiveAction('Recurring'); } },
  ...
  async function submitEntry(event: FormEvent<HTMLFormElement>) {
    ...
    if (activeAction === 'Transfer') {
      const sourceAccount = data.accounts.find((account) => account.id === payload.fromAccountId);
      const destinationAccount = data.accounts.find((account) => account.id === payload.toAccountId);
      const sourceIsCash = sourceAccount ? normalizeAccountType(sourceAccount.accountType) === 'cash' : false;
      const destinationIsCash = destinationAccount ? normalizeAccountType(destinationAccount.accountType) === 'cash' : false;
      if (!sourceIsCash && destinationIsCash) { payload.description = 'Bank withdrawal'; payload.category = 'Cash movement'; payload.paymentMethod = 'Cash'; }
      if (sourceIsCash && !destinationIsCash) { payload.description = 'Bank deposit'; payload.category = 'Cash movement'; payload.paymentMethod = 'Cash'; }
    }
  ...
  async function submitCardEntry(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setData(result); setCardModal(null); setToast(cardModal === 'account' ? selectedAccountId ? 'Account updated' : 'Account added' : cardModal === 'bankLoan' ? 'Bank loan added' : cardModal === 'card' ? 'Credit card added' : cardModal === 'statement' ? 'Statement updated' : cardModal === 'loan' ? 'Personal loan saved' : cardModal === 'cardPayment' ? 'Card payment saved' : cardModal === 'loanPayment' ? 'Loan payment saved' : cardModal === 'loanIncrease' ? 'Additional debt saved' : cardModal === 'sharedContribution' ? 'Contribution received' : 'Card purchase saved');
      if (receivedPersonalPayment || cardModal === 'sharedContribution') celebrateMoneyIn();
      if (cardModal === 'account') { setSelectedAccountId(''); setAccountPreset('bank'); }
      window.setTimeout(() => setToast(''), 2400);
  ...
  return (
    <main ref={root} className={`app-shell ${loading ? 'app-is-loading' : ''}`}>
      ...
      <section className={`dashboard ${loading ? 'is-loading' : ''}`} id="top" aria-busy={loading}>
        ...
        {error && !activeAction && <div className="page-error" role="alert"><span><strong>{/connect|load your financial data/i.test(error) ? tr('Connection interrupted', 'Conexión interrumpida') : tr('Movement not completed', 'Movimiento no realizado')}</strong><small>{localizedError(error)}</small></span>{/connect|load your financial data/i.test(error) ? <button type="button" onClick={() => { setLoading(true); setError(''); setLoadAttempt((attempt) => attempt + 1); }}>{tr('Try again', 'Reintentar')}</button> : <button type="button" onClick={() => setError('')}>{tr('Dismiss', 'Cerrar')}</button>}</div>}
        ...
        <section className="hero-grid" id="overview" hidden={activeView !== 'overview'}>
          ...
          <article className="funding-card" data-reveal>
            ...
            <div className="funding-main"><strong>{money(totalFundingNeeded)}</strong><span>{tr(`still needed across ${fundingPlan.length} accounts`, `aún necesarios en ${fundingPlan.length} cuentas`)}</span></div>
            <div className="progress-track"><span style={{ width: `${Math.min(100, totalReadyForPayments / Math.max(outstanding, 1) * 100)}%` }} /></div>
            <div className="funding-meta"><span>{money(totalReadyForPayments)} {tr('ready', 'disponibles')}</span><span>{money(outstanding)} {tr('scheduled', 'programados')}</span></div>
            <button className="funding-plan-button" type="button" onClick={() => setShowFundingPlan(true)}>{tr('View account plan', 'Ver plan por cuenta')} <span>→</span></button>
          </article>
        </section>
        ...
        {showBalanceDetail && <div className="modal-backdrop balance-detail-backdrop" role="presentation" onMouseDown={() => setShowBalanceDetail(false)}>
          <section className="modal balance-detail-modal" role="dialog" aria-modal="true" aria-labelledby="balance-detail-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="balance-account-list">{availableAccounts.map((account) => {
              ...
              const identity = bankIdentity(account.bank);
              return <article className={`bank-theme bank-${identity.key}`} key={account.id}><span className="balance-account-icon"><BankMark bank={account.bank}/></span><span><strong>{account.name}</strong><small>{account.bank} · {accountTypeLabel(account)}</small><i><b style={{ width: `${Math.min(100, share)}%` }} /></i></span><span><strong>{money(account.balance)}</strong><small>{totalBalance > 0 ? `${share.toFixed(0)}%` : '—'}</small></span></article>;
        ...
        <section className="bank-loans-panel panel" data-reveal hidden={activeView !== 'credit'}>
          ...
          {data.bankLoans.length ? <div className="bank-loan-grid">{data.bankLoans.map((loan) => {
            ...
            const identity = bankIdentity(loan.bank);
            return <article className={`bank-theme bank-${identity.key}`} key={loan.id}><div className="bank-loan-top"><span><small><i className="loan-bank-mark"><BankMark bank={loan.bank}/></i>{loan.bank}</small><strong>{loan.name}</strong></span><button className="danger-mini icon-action" type="button" aria-label={tr(`Delete ${loan.name}`, `Eliminar ${loan.name}`)} title={tr('Delete', 'Eliminar')} onClick={() => deleteBankLoan(loan)}><TrashIcon /></button></div><div className="bank-loan-values"><span><small>{tr('Outstanding balance', 'Saldo pendiente')}</small><strong>{money(loan.outstandingBalance)}</strong></span><span><small>{tr('Monthly installment', 'Cuota mensual')}</small><strong>{money(loan.installment)}</strong></span><span><small>{tr('Next payment', 'Próximo pago')}</small><strong>{shortDate(loan.nextDueDate)}</strong></span></div><div className="bank-loan-track"><i style={{ width: `${progress}%` }} /></div><div className="bank-loan-meta"><small>{tr(`${loan.paidInstallments} of ${loan.totalInstallments} installments paid`, `${loan.paidInstallments} de ${loan.totalInstallments} cuotas pagadas`)}</small><small>{tr('Automatic Debit', 'Débito automático')} · {accountMap.get(bankLoanAccountId(loan) || '') || tr(`day ${loan.paymentDay}`, `día ${loan.paymentDay}`)}</small></div></article>;
        ...
        <section className="loans-panel panel" data-reveal hidden={activeView !== 'people'}>
          ...
          {openPersonalLoans.length ? <div className="loan-grid">{openPersonalLoans.map((loan) => {
            const history = (data.personalLoanPayments || []).filter((payment) => payment.personalLoanId === loan.id);
            return <article className={`${loan.direction} ${loan.status === 'settled' ? 'settled' : ''}`} key={loan.id}><span>{loan.status === 'settled' ? '✓' : loan.direction === 'i_owe' ? '↓' : '↑'}</span><div><small>{loan.status === 'settled' ? tr('Settled', 'Saldado') : loan.direction === 'i_owe' ? tr('I owe', 'Yo debo') : tr('Owed to me', 'Me debe')}</small><strong>{loan.personName}</strong><p>{loan.note || (loan.direction === 'i_owe' ? tr('Money I need to return', 'Dinero que debo devolver') : tr('Money they need to return', 'Dinero que deben devolverme'))}</p></div><div><strong>{money(Math.max(loan.amount - loan.paid, 0))}</strong><span className="loan-actions"><button type="button" onClick={() => { setSelectedCardId(loan.id); setCardModal('loanIncrease'); }}>{tr('Add debt', 'Aumentar deuda')}</button>{loan.status !== 'settled' && <button type="button" onClick={() => { setSelectedCardId(loan.id); setCardModal('loanPayment'); }}>{tr('Record payment', 'Registrar pago')}</button>}<button className="history-toggle" type="button" aria-label={expandedLoanId === loan.id ? tr('Hide movement history', 'Ocultar historial') : tr('Show movement history', 'Mostrar historial')} title={expandedLoanId === loan.id ? tr('Hide history', 'Ocultar historial') : tr('Show history', 'Mostrar historial')} onClick={() => setExpandedLoanId((current) => current === loan.id ? null : loan.id)}><ChevronIcon open={expandedLoanId === loan.id} />{history.length > 0 && <i>{history.length}</i>}</button><button className="loan-delete icon-action" type="button" aria-label={tr(`Delete IOU with ${loan.personName}`, `Eliminar deuda con ${loan.personName}`)} title={tr('Delete', 'Eliminar')} onClick={() => deletePersonalLoan(loan)}><TrashIcon /></button></span></div>{expandedLoanId === loan.id && <div className="loan-history"><p>{tr('Movement history', 'Historial de movimientos')}</p>{history.length ? history.slice(0, 8).map((payment) => <span key={payment.id}><small>{shortDate(payment.date)} · {accountMap.get(payment.accountId) || tr('Account', 'Cuenta')}</small><strong>{payment.entryType === 'advance' ? `${tr('Added', 'Agregado')} ${money(payment.amount)}` : `${loan.direction === 'i_owe' ? tr('You paid', 'Pagaste') : tr('You received', 'Recibiste')} ${money(payment.amount)}`}</strong><button className="history-delete icon-action" type="button" aria-label={tr('Delete this movement', 'Eliminar este movimiento')} title={tr('Delete', 'Eliminar')} onClick={() => deleteLoanPayment(payment)}><TrashIcon /></button></span>) : <small>{tr('No payments or additional loans recorded yet.', 'Todavía no hay pagos ni aumentos de deuda registrados.')}</small>}</div>}</article>;
          })}</div> : <div className="credit-empty"><span>↔</span><div><strong>{tr('No personal loans pending', 'No hay deudas personales pendientes')}</strong><small>{tr('Record money borrowed from friends or money you lent so it is not forgotten.', 'Registra lo que te prestaron o lo que prestaste para no perderlo de vista.')}</small></div><button type="button" onClick={() => setCardModal('loan')}>{tr('Add IOU', 'Agregar deuda')}</button></div>}
          {sharedPayments.length > 0 && <div className="shared-payments"><div className="shared-payments-heading"><span><p className="eyebrow">{tr('SHARED PAYMENTS', 'PAGOS COMPARTIDOS')}</p><h3>{tr('Contributions to collect', 'Aportes por cobrar')}</h3></span><small>{tr('Current month', 'Mes actual')}</small></div>{sharedPayments.map((payment) => { const contributions = data.sharedContributions.filter((item) => item.recurringPaymentId === payment.id && item.cycleMonth === currentCycleMonth); const collected = contributions.reduce((sum, item) => sum + item.amount, 0); const externalShare = payment.sharedMembers.reduce((sum, member) => sum + member.amount, 0); return <article className="shared-payment-card" key={`shared-${payment.id}`}><header><span><small>{accountMap.get(payment.pay_from_account_id)}</small><strong>{payment.name}</strong></span><span><small>{tr('Full debit', 'Débito total')}</small><strong>{money(payment.amount)}</strong></span></header><div className="shared-payment-summary"><span><small>{tr('Your share', 'Tu parte')}</small><strong>{money(Math.max(0, payment.amount - externalShare))}</strong></span><span><small>{tr('Collected', 'Cobrado')}</small><strong>{money(collected)}</strong></span><span><small>{tr('Pending', 'Pendiente')}</small><strong>{money(Math.max(0, externalShare - collected))}</strong></span></div><div className="shared-member-list">{payment.sharedMembers.map((member) => { const contribution = contributions.find((item) => item.participantName === member.name); return <div className={contribution ? 'paid' : ''} key={`${payment.id}-${member.name}`}><span><i>{contribution ? '✓' : member.name[0]}</i><span><strong>{member.name}</strong><small>{contribution ? `${tr('Received', 'Recibido')} ${shortDate(contribution.receivedDate)}` : tr('Awaiting contribution', 'Aporte pendiente')}</small></span></span><strong>{money(member.amount)}</strong>{contribution ? <b>{tr('Paid', 'Pagó')}</b> : <button type="button" onClick={() => { setSelectedCardId(payment.id); setSelectedSharedParticipant(member.name); setCardModal('sharedContribution'); }}>{tr('Record payment', 'Registrar pago')}</button>}</div>; })}</div></article>; })}</div>}
        </section>
        ...
        <section className="activity-panel panel" id="activity" data-reveal hidden={activeView !== 'activity'}>
          ...
          <div className="activity-grid">
              ...
              {categorySpending.length ? <>
                <div className="category-chart">{categorySpending.map((item, index) => {
                  ...
                  return <div className="category-row" key={item.category}>
                    <div className="category-meta"><span><i>{index + 1}</i><strong>{transactionCategory(item.category)}</strong><small>{item.count} {tr(item.count === 1 ? 'purchase' : 'purchases', item.count === 1 ? 'compra' : 'compras')}</small></span><span><strong>{money(item.amount)}</strong><small>{percentage.toFixed(0)}% {tr('of spending', 'del gasto')}</small></span></div>
            ...
            <div className="activity-list"><form className="activity-date-filter" onSubmit={(event) => { event.preventDefault(); setActivityStartDate(activityDraftStartDate); setActivityEndDate(activityDraftEndDate); setActivityPage(1); }}><span><small>{tr('FROM', 'DESDE')}</small><input type="date" value={activityDraftStartDate} max={activityDraftEndDate} onChange={(event) => { const value = event.target.value; if (!value) return; setActivityDraftStartDate(value); if (value > activityDraftEndDate) setActivityDraftEndDate(value); }}/></span><span><small>{tr('TO', 'HASTA')}</small><input type="date" value={activityDraftEndDate} min={activityDraftStartDate} onChange={(event) => { const value = event.target.value; if (!value) return; setActivityDraftEndDate(value); if (value < activityDraftStartDate) setActivityDraftStartDate(value); }}/></span><span className="activity-filter-actions"><button type="submit">{tr('Apply', 'Aplicar')}</button><button type="button" onClick={() => { setActivityDraftStartDate(today); setActivityDraftEndDate(today); setActivityStartDate(today); setActivityEndDate(today); setActivityPage(1); }}>{tr('Today', 'Hoy')}</button></span><b>{filteredActivity.length} {tr(filteredActivity.length === 1 ? 'movement' : 'movements', filteredActivity.length === 1 ? 'movimiento' : 'movimientos')}</b></form>
              ...
              {filteredActivity.length > 10 && <nav className="activity-pagination" aria-label={tr('Movement pages', 'Páginas de movimientos')}><button type="button" disabled={visibleActivityPage === 1} onClick={() => setActivityPage((page) => Math.max(1, page - 1))}>← {tr('Previous', 'Anterior')}</button><span>{tr('Page', 'Página')} <strong>{visibleActivityPage}</strong> {tr('of', 'de')} {activityPageCount}</span><button type="button" disabled={visibleActivityPage === activityPageCount} onClick={() => setActivityPage((page) => Math.min(activityPageCount, page + 1))}>{tr('Next', 'Siguiente')} →</button></nav>}
            </div>
          </div>
        </section>
        ...
        <section className="content-grid single-view" hidden={activeView !== 'accounts' && activeView !== 'payments'}>
          <article className="panel" id="accounts" data-reveal hidden={activeView !== 'accounts'}>
            <div className="section-heading"><div><p className="eyebrow">{tr('YOUR MONEY', 'TU DINERO')}</p><h2>{tr('Accounts', 'Cuentas')}</h2></div><div className="section-actions"><span className="live-label">{tr('Live balances', 'Saldos actuales')}</span><button type="button" onClick={() => { setSelectedAccountId(''); setAccountPreset('bank'); setCardModal('account'); }}>+ {tr('Add account', 'Agregar cuenta')}</button></div></div>
            <div className="account-list account-bank-list">{accountsByBank.map((group, bankIndex) => { const identity = bankIdentity(group.bank); return <section className={`account-bank-group bank-theme bank-${identity.key}`} key={group.bank}>
              <header className="account-bank-heading"><i className="bank-mark"><BankMark bank={group.bank}/></i><span><small>{tr('BANK', 'BANCO')}</small><strong>{group.bank}</strong></span><span><strong>{money(group.balance)}</strong><small>{group.accounts.length} {tr(group.accounts.length === 1 ? 'account' : 'accounts', group.accounts.length === 1 ? 'cuenta' : 'cuentas')}</small></span></header>
              {group.accounts.map((account, accountIndex) => { const movements = data.transactions.filter((item) => item.from_account_id === account.id || item.to_account_id === account.id); const expanded = expandedAccountId === account.id; const colorIndex = bankIndex + accountIndex; return <div className={`account-entry ${expanded ? 'expanded' : ''}`} key={account.id}><div className="account-row"><span className={`account-icon ${['mint', 'violet', 'orange'][colorIndex % 3]}`}>{accountInitial(account.name)}</span><span><strong>{account.name}</strong><small>{account.accountType}</small></span><strong>{money(account.balance)}</strong><span className="row-actions"><button className="history-toggle icon-action" type="button" aria-expanded={expanded} aria-label={expanded ? tr(`Hide movements for ${account.name}`, `Ocultar movimientos de ${account.name}`) : tr(`Show movements for ${account.name}`, `Mostrar movimientos de ${account.name}`)} title={tr('Movements', 'Movimientos')} onClick={() => setExpandedAccountId((current) => current === account.id ? null : account.id)}><ChevronIcon open={expanded}/>{movements.length > 0 && <i>{movements.length}</i>}</button><button className="icon-action" type="button" aria-label={tr(`Edit ${account.name}`, `Editar ${account.name}`)} title={tr('Edit', 'Editar')} onClick={() => { setSelectedAccountId(account.id); setCardModal('account'); }}><PencilIcon /></button><button className="danger-mini icon-action" type="button" aria-label={tr(`Delete ${account.name}`, `Eliminar ${account.name}`)} title={tr('Delete', 'Eliminar')} onClick={() => deleteAccount(account)}><TrashIcon /></button></span></div>{expanded && <div className="account-movement-history"><header><span><small>{tr('ACCOUNT ACTIVITY', 'ACTIVIDAD DE LA CUENTA')}</small><strong>{tr('Movements', 'Movimientos')}</strong></span><b>{movements.length}</b></header>{movements.length ? <div>{movements.map((item) => { const incoming = item.to_account_id === account.id; const otherAccount = accountMap.get(incoming ? item.from_account_id || '' : item.to_account_id || ''); return <span className="account-movement" key={`${account.id}-${item.id}`}><i className={incoming ? 'incoming' : 'outgoing'}>{incoming ? '↓' : '↑'}</i><span><strong>{transactionDescription(item.description)}</strong><small>{shortDate(item.date)} · {item.type === 'transfer' ? `${incoming ? tr('From', 'Desde') : tr('To', 'Hacia')} ${otherAccount || tr('another account', 'otra cuenta')}` : transactionCategory(item.category) || item.payment_method || item.type}</small></span><b className={incoming ? 'incoming' : 'outgoing'}>{incoming ? '+' : '−'}{money(item.amount)}</b></span>; })}</div> : <div className="account-history-empty"><strong>{tr('No movements yet', 'Todavía no hay movimientos')}</strong><small>{tr('Movements involving this account will appear here.', 'Aquí aparecerán los movimientos relacionados con esta cuenta.')}</small></div>}</div>}</div>; })}
            </section>; })}</div>
          </article>
          <article className="panel" id="plans" data-reveal hidden={activeView !== 'payments'}>
            <div className="section-heading"><div><p className="eyebrow">{tr('MONTHLY PLAN', 'PLAN MENSUAL')}</p><h2>{tr('Income & commitments', 'Ingresos y compromisos')}</h2></div><div className="section-actions"><span className="live-label">{allUpcoming.length + planningCardSummaries.length + planningBankLoans.length} {tr('in this cycle', 'en este ciclo')}</span><button type="button" onClick={() => { setEditingRecurring(null); setEditingTransaction(null); setRecurringFlow('expense'); setSharedPaymentEnabled(false); setActiveAction('Recurring'); }}>+ {tr('Schedule', 'Programar')}</button></div></div>
            ...
            <div className="upcoming-list">{upcoming.map((payment) => <div className="payment-row editable-payment-row" key={payment.id}><span className="payment-icon">◷</span><span><strong>{payment.name}</strong><small>{shortDate(payment.next_due_date)} · {accountMap.get(payment.pay_from_account_id)}</small></span><strong>−{money(payment.amount)}</strong><span className="row-actions"><button className="icon-action" type="button" aria-label={tr(`Edit ${payment.name}`, `Editar ${payment.name}`)} title={tr('Edit', 'Editar')} onClick={() => { setEditingRecurring(payment); setRecurringFlow('expense'); setActiveAction('Recurring'); }}><PencilIcon /></button><button className="danger-mini icon-action" type="button" aria-label={`Delete ${payment.name}`} title={tr('Delete', 'Eliminar')} onClick={() => deleteRecurring(payment)}><TrashIcon /></button></span></div>)}{planningBankLoans.map((loan) => <div className="payment-row bank-loan-payment-row" key={`loan-${loan.id}`}><span className="payment-icon">▤</span><span><strong>{loan.name}</strong><small>{shortDate(loan.nextDueDate)} · Automatic Debit · {accountMap.get(bankLoanAccountId(loan) || '') || loan.bank}</small></span><strong>−{money(loan.installment)}</strong></div>)}{planningCardSummaries.map(({ card, estimatedPayment, nextPaymentDate }) => <div className="payment-row card-payment-row" key={`card-${card.id}`}><span className="payment-icon">◇</span><span><strong>{card.name} payment</strong><small>{shortDate(nextPaymentDate)} · Bank Transfer · {accountMap.get(card.payFromAccountId || '') || 'select payment account'}</small></span><strong>−{money(estimatedPayment)}</strong></div>)}{!upcoming.length && !planningCardSummaries.length && !planningBankLoans.length && <div className="empty-state"><strong>No upcoming payments</strong><small>Add a recurring payment, loan or credit card above.</small></div>}</div>
          ...
          <div className="modal-backdrop" role="presentation" onMouseDown={() => { setShowFundingPlan(false); setExpandedFundingAccount(null); }}>
            <section className={`modal-card funding-plan-modal ${activeFundingMonth === nextMonthKey ? 'next-month' : ''}`} role="dialog" aria-modal="true" aria-labelledby="funding-title" onMouseDown={(event) => event.stopPropagation()}>
              ...
              <div className="funding-account-list">{fundingPlan.map((item) => <article className="funding-account-row" key={item.account.id}>
                <button className="funding-account-toggle" type="button" aria-expanded={expandedFundingAccount === item.account.id} onClick={() => setExpandedFundingAccount((current) => current === item.account.id ? null : item.account.id)}><span className="funding-account-icon">{accountInitial(item.account.name)}</span><span><strong>{item.account.name}</strong><small>{item.paymentCount} {item.paymentCount === 1 ? 'payment' : 'payments'} · due from {item.nextDue ? shortDate(item.nextDue) : '—'}</small></span><i>⌄</i></button>
        ...
        {activeAction && (
          <div className="modal-backdrop" role="presentation" onMouseDown={() => { if (!saving) { setActiveAction(null); setEditingTransaction(null); setEditingRecurring(null); } }}>
            <section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="modal-title" onMouseDown={(event) => event.stopPropagation()}>
              ...
              <form onSubmit={submitEntry} onChange={(event) => { const target = event.nativeEvent.target; if (!(target instanceof HTMLSelectElement)) return; if (target.name === 'paymentMethod' && target.value === 'Cash' && cashAccounts[0]) { const source = event.currentTarget.elements.namedItem('fromAccountId') as HTMLSelectElement | null; if (source) source.value = cashAccounts[0].id; } }}>
                {activeAction === 'Recurring' && <label><span>{tr('Schedule type', 'Tipo de programación')}</span><select name="flowType" value={recurringFlow} onChange={(event) => setRecurringFlow(event.target.value as 'income' | 'expense')}><option value="expense">{tr('Recurring expense', 'Gasto recurrente')}</option><option value="income">{tr('Recurring income', 'Ingreso recurrente')}</option></select></label>}
        ...
        {cardModal && <div className="modal-backdrop" role="presentation" onMouseDown={() => { if (!saving) { setCardModal(null); setAccountPreset('bank'); } }}>
          <section className={`modal-card ${cardModal === 'account' && accountPreset === 'cash' ? 'cash-account-modal' : ''}`} role="dialog" aria-modal="true" aria-labelledby="card-modal-title" onMouseDown={(event) => event.stopPropagation()}>
            <div className="modal-heading"><div><p className="eyebrow">{cardModal === 'sharedContribution' ? tr('SHARED PAYMENT', 'PAGO COMPARTIDO') : cardModal === 'account' ? tr('ACCOUNT SETUP', 'CONFIGURACIÓN DE CUENTA') : cardModal === 'bankLoan' ? tr('BANK DEBT', 'DEUDA BANCARIA') : cardModal === 'loan' || cardModal === 'loanPayment' || cardModal === 'loanIncrease' ? tr('PERSONAL IOU', 'DEUDA PERSONAL') : tr('CREDIT CONTROL', 'CONTROL DE CRÉDITO')}</p><h2 id="card-modal-title">{cardModal === 'sharedContribution' ? tr('Record contribution', 'Registrar aporte') : cardModal === 'account' ? accountPreset === 'cash' ? selectedAccountId ? tr('Edit cash wallet', 'Editar efectivo') : tr('Add cash wallet', 'Agregar efectivo') : selectedAccountId ? tr('Edit bank account', 'Editar cuenta bancaria') : tr('Add bank account', 'Agregar cuenta bancaria') : cardModal === 'bankLoan' ? tr('Add bank loan', 'Agregar préstamo bancario') : cardModal === 'card' ? tr('Add credit card', 'Agregar tarjeta de crédito') : cardModal === 'statement' ? tr('Card settings', 'Configuración de tarjeta') : cardModal === 'loan' ? tr('Track borrowed money', 'Registrar dinero entre personas') : cardModal === 'loanPayment' ? tr('Record loan payment', 'Registrar pago de deuda') : cardModal === 'loanIncrease' ? tr('Add to existing debt', 'Aumentar deuda existente') : cardModal === 'cardPayment' ? tr('Pay credit card', 'Pagar tarjeta de crédito') : tr('New card purchase', 'Nueva compra con tarjeta')}</h2></div><button onClick={() => { setCardModal(null); setSelectedAccountId(''); setAccountPreset('bank'); setSelectedSharedParticipant(''); }} aria-label={tr('Close', 'Cerrar')} disabled={saving}>×</button></div>
            <form onSubmit={submitCardEntry}>
              {cardModal === 'sharedContribution' ? <><input type="hidden" name="recurringPaymentId" value={selectedCardId}/><input type="hidden" name="participantName" value={selectedSharedParticipant}/><input type="hidden" name="cycleMonth" value={currentCycleMonth}/><p className="loan-modal-summary">{selectedSharedParticipant} · {data.recurring.find((item) => item.id === selectedCardId)?.name}</p><label><span>{tr('Receive in account', 'Recibir en la cuenta')}</span><select name="accountId" required defaultValue=""><option value="" disabled>{tr('Select account', 'Selecciona una cuenta')}</option>{data.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><div className="field-row"><label><span>{tr('Amount received', 'Valor recibido')}</span><input name="amount" required autoFocus type="text" inputMode="decimal" defaultValue={data.recurring.find((item) => item.id === selectedCardId)?.sharedMembers.find((member) => member.name === selectedSharedParticipant)?.amount || ''}/></label><label><span>{tr('Date received', 'Fecha recibida')}</span><input name="date" required type="date" defaultValue={today}/></label></div></> : cardModal === 'account' ? <>
                {accountPreset === 'cash' && <div className="cash-modal-intro"><span>$</span><p><strong>{tr('Your physical wallet', 'Tu billetera física')}</strong><small>{tr('Update this balance only when counting cash. Use transfers for withdrawals and deposits.', 'Ajusta este saldo solo cuando cuentes el efectivo. Usa transferencias para retiros y depósitos.')}</small></p></div>}
                <label><span>{tr('Account name', 'Nombre de la cuenta')}</span><input name="name" required autoFocus defaultValue={data.accounts.find((account) => account.id === selectedAccountId)?.name || (accountPreset === 'cash' ? tr('Cash wallet', 'Efectivo') : '')} placeholder={accountPreset === 'cash' ? tr('Cash wallet', 'Efectivo') : 'Produbanco Savings'} /></label>
                {accountPreset === 'cash' ? <input type="hidden" name="bank" value="Cash" /> : <label><span>{tr('Bank', 'Banco')}</span><select name="bank" required defaultValue={data.accounts.find((account) => account.id === selectedAccountId)?.bank || 'Pichincha'}><option>Pichincha</option><option>Produbanco</option><option>Pacifico</option><option>Guayaquil</option></select></label>}
                <div className="field-row">{accountPreset === 'cash' ? <input type="hidden" name="accountType" value="Cash" /> : <label><span>{tr('Account type', 'Tipo de cuenta')}</span><select name="accountType" required defaultValue={data.accounts.find((account) => account.id === selectedAccountId)?.accountType || 'Savings'}>{['Savings', 'Checking', 'Debit'].map((value) => <option key={value} value={value}>{accountTypeLabels[value]}</option>)}</select></label>}<label><span>{selectedAccountId ? tr('Current balance', 'Saldo actual') : tr('Starting balance', 'Saldo inicial')}</span><input name="balance" required type="text" inputMode="decimal" defaultValue={selectedAccountId ? data.accounts.find((account) => account.id === selectedAccountId)?.balance ?? 0 : 0} /></label></div>
              </> : cardModal === 'bankLoan' ? <>
                <label><span>{tr('Loan name', 'Nombre del préstamo')}</span><input name="name" required autoFocus placeholder="Pichincha Preciso" /></label>
                <label><span>{tr('Bank', 'Banco')}</span><select name="bank" required defaultValue=""><option value="" disabled>{tr('Select bank', 'Selecciona un banco')}</option><option>Pichincha</option><option>Produbanco</option><option>Pacifico</option><option>Guayaquil</option></select></label>
                ...
                <div className="field-row"><label><span>{tr('Network', 'Franquicia')}</span><select name="network" required defaultValue="Visa"><option>Visa</option><option>Mastercard</option><option>Discover</option><option>Amex</option><option value="Other">{tr('Other', 'Otra')}</option></select></label><label><span>{tr('Pay from account', 'Pagar desde la cuenta')}</span><select name="payFromAccountId" defaultValue=""><option value="">{tr('Choose later', 'Elegir después')}</option>{data.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label></div>
                <div className="field-row"><label><span>{tr('Credit limit', 'Límite de crédito')}</span><input name="creditLimit" required type="text" inputMode="decimal" placeholder="3300,00" /></label><label><span>{tr('Currently used', 'Uso actual')}</span><input name="openingUsed" required type="text" inputMode="decimal" placeholder="1340,76" /></label></div>
                <div className="field-row"><label><span>{tr('Current statement due (optional)', 'Valor del estado de cuenta (opcional)')}</span><input name="currentStatement" type="text" inputMode="decimal" defaultValue="0" /></label><label><span>{tr('Statement day', 'Día de corte')}</span><input name="statementDay" type="number" min="1" max="31" placeholder="25" /></label></div><label><span>{tr('Payment day', 'Día de pago')}</span><input name="paymentDay" type="number" min="1" max="31" placeholder="15" /></label>
                <small className="rate-note">{tr("The estimated interest rate is assigned automatically from the selected bank's current consumer-credit reference.", 'La tasa de interés estimada se asigna automáticamente según la referencia vigente de crédito de consumo del banco seleccionado.')}</small>
              </> : cardModal === 'statement' ? <><label><span>Credit card</span><select name="creditCardId" required defaultValue={selectedCardId || data.creditCards[0]?.id || ''}>{data.creditCards.map((card) => <option key={card.id} value={card.id}>{card.name}</option>)}</select></label><label><span>Statement amount due (optional)</span><input name="currentStatement" autoFocus type="text" inputMode="decimal" defaultValue={data.creditCards.find((card) => card.id === selectedCardId)?.currentStatement ?? 0} placeholder="0.00" /></label><div className="field-row"><label><span>Credit limit</span><input name="creditLimit" type="text" inputMode="decimal" defaultValue={data.creditCards.find((card) => card.id === selectedCardId)?.creditLimit} /></label><label><span>Statement day</span><input name="statementDay" type="number" min="1" max="31" defaultValue={data.creditCards.find((card) => card.id === selectedCardId)?.statementDay || ''} /></label></div><div className="field-row"><label><span>Payment day</span><input name="paymentDay" type="number" min="1" max="31" defaultValue={data.creditCards.find((card) => card.id === selectedCardId)?.paymentDay || ''} /></label><label><span>Network</span><select name="network" defaultValue={data.creditCards.find((card) => card.id === selectedCardId)?.network || 'Visa'}><option>Visa</option><option>Mastercard</option><option>Discover</option><option>Amex</option><option>Other</option></select></label></div><label><span>Pay from account</span><select name="payFromAccountId" required defaultValue={data.creditCards.find((card) => card.id === selectedCardId)?.payFromAccountId || ''}><option value="" disabled>Select account</option>{data.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label></> : cardModal === 'loan' ? <><label><span>{tr('Direction', 'Dirección')}</span><select name="direction" required defaultValue="i_owe"><option value="i_owe">{tr('Someone lent me money — I owe', 'Me prestaron dinero — yo debo')}</option><option value="owed_to_me">{tr('I lent money — they owe me', 'Yo presté dinero — me deben')}</option></select></label><label><span>{tr('Person', 'Persona')}</span><input name="personName" required autoFocus placeholder={tr('Friend or family member', 'Amigo o familiar')} /></label><label><span>{tr('Balance impact', 'Impacto en el saldo')}</span><select name="accountId" defaultValue=""><option value="">{tr('Track debt only — do not change an account', 'Solo registrar la deuda — no cambiar una cuenta')}</option>{data.accounts.map((account) => <option key={account.id} value={account.id}>{tr('Money entered/left', 'El dinero entró/salió de')} {account.name}</option>)}</select><small className="field-help">{tr('Choose an account only when this loan actually moved money into or out of it.', 'Elige una cuenta únicamente si este préstamo realmente hizo entrar o salir dinero.')}</small></label><div className="field-row"><label><span>{tr('Amount', 'Monto')}</span><input name="amount" required type="text" inputMode="decimal" /></label><label><span>{tr('Due date (optional)', 'Fecha límite (opcional)')}</span><input name="dueDate" type="date" /></label></div><label><span>{tr('Note (optional)', 'Nota (opcional)')}</span><input name="note" placeholder={tr('What was the money for?', '¿Para qué fue el dinero?')} /></label></> : cardModal === 'cardPayment' ? <><input type="hidden" name="creditCardId" value={selectedCardId} /><label><span>Pay from account</span><select name="fromAccountId" required defaultValue={data.creditCards.find((card) => card.id === selectedCardId)?.payFromAccountId || ''}><option value="" disabled>Select account</option>{data.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><div className="field-row"><label><span>Amount</span><input name="amount" required autoFocus type="text" inputMode="decimal" defaultValue={data.creditCards.find((card) => card.id === selectedCardId)?.currentStatement || ''} /></label><label><span>Date</span><input name="date" required type="date" defaultValue={today} /></label></div><label><span>Note</span><input name="note" placeholder="Statement payment" /></label></> : cardModal === 'loanPayment' ? <><input type="hidden" name="personalLoanId" value={selectedCardId} /><label><span>{tr('Account', 'Cuenta')}</span><select name="accountId" required defaultValue={data.personalLoans.find((loan) => loan.id === selectedCardId)?.accountId || ''}><option value="" disabled>{tr('Select account', 'Selecciona una cuenta')}</option>{data.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><div className="field-row"><label><span>{tr('Amount', 'Monto')}</span><input name="amount" required autoFocus type="text" inputMode="decimal" defaultValue={(data.personalLoans.find((loan) => loan.id === selectedCardId)?.amount || 0) - (data.personalLoans.find((loan) => loan.id === selectedCardId)?.paid || 0)} /></label><label><span>{tr('Date', 'Fecha')}</span><input name="date" required type="date" defaultValue={today} /></label></div></> : cardModal === 'loanIncrease' ? <><input type="hidden" name="personalLoanId" value={selectedCardId} /><p className="loan-modal-summary">{tr(`Add more to ${data.personalLoans.find((loan) => loan.id === selectedCardId)?.personName}'s existing balance.`, `Aumenta el saldo pendiente de ${data.personalLoans.find((loan) => loan.id === selectedCardId)?.personName}.`)}</p><label><span>{tr('Account involved', 'Cuenta involucrada')}</span><select name="accountId" required defaultValue={data.personalLoans.find((loan) => loan.id === selectedCardId)?.accountId || ''}><option value="" disabled>{tr('Select account', 'Selecciona una cuenta')}</option>{data.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><div className="field-row"><label><span>{tr('Additional amount', 'Monto adicional')}</span><input name="amount" required autoFocus type="text" inputMode="decimal" /></label><label><span>{tr('Date', 'Fecha')}</span><input name="date" required type="date" defaultValue={today} /></label></div></> : <>
              ...
              {error && <small className="form-error" role="alert">{localizedError(error)}</small>}
              <button className="save-button" type="submit" disabled={saving}>{saving ? tr('Saving…', 'Guardando…') : cardModal === 'sharedContribution' ? tr('Save contribution', 'Guardar aporte') : cardModal === 'account' ? accountPreset === 'cash' ? selectedAccountId ? tr('Update cash balance', 'Actualizar efectivo') : tr('Create cash wallet', 'Crear billetera de efectivo') : selectedAccountId ? tr('Save account changes', 'Guardar cambios de cuenta') : tr('Add account', 'Agregar cuenta') : cardModal === 'bankLoan' ? tr('Add bank loan', 'Agregar préstamo bancario') : cardModal === 'card' ? tr('Add credit card', 'Agregar tarjeta de crédito') : cardModal === 'statement' ? tr('Save card settings', 'Guardar configuración') : cardModal === 'loan' ? tr('Save personal IOU', 'Guardar deuda personal') : cardModal === 'cardPayment' ? tr('Save card payment', 'Guardar pago de tarjeta') : cardModal === 'loanPayment' ? tr('Save loan payment', 'Guardar pago de deuda') : cardModal === 'loanIncrease' ? tr('Add to debt', 'Aumentar deuda') : tr('Save card purchase', 'Guardar compra')}</button>
              <small className="demo-note">{cardModal === 'account' && accountPreset === 'cash' ? tr('Withdrawals and deposits should be recorded as transfers so both balances stay correct.', 'Los retiros y depósitos deben registrarse como transferencias para mantener correctos ambos saldos.') : tr('Payment values are estimates. Your bank statement remains the final source.', 'Los valores registrados son estimados. Tu estado de cuenta bancario sigue siendo la fuente definitiva.')}</small>
            </form>
          </section>
        </div>}
