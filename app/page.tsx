...
export default function Home() {
  ...
  async function detachPersonalLoanAccount(loan: PersonalLoan) {
    if (!window.confirm(tr(`Stop ${loan.personName}'s IOU from affecting the selected account balance?`, `¿Dejar de contar la deuda con ${loan.personName} en el saldo de la cuenta seleccionada?`))) return;
    const response = await authenticatedFetch('/api/dashboard', { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ personalLoanId: loan.id, action: 'detachAccount' }) });
    const result = await response.json();
    if (response.ok) { setData(result); setToast(tr('Account balance corrected', 'Saldo de la cuenta corregido')); window.setTimeout(() => setToast(''), 2400); }
    else setError(result.error || tr('Unable to correct this balance.', 'No se pudo corregir este saldo.'));
  }
          ...
          <div className="modal-backdrop" role="presentation" onMouseDown={() => { if (!saving) { setActiveAction(null); setEditingTransaction(null); setEditingRecurring(null); } }}>
            <section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="modal-title" onMouseDown={(event) => event.stopPropagation()}>
              ...
              <form onSubmit={submitEntry}>
                ...
                <label><span>{tr('Description', 'Descripción')}</span><input name="description" required autoFocus defaultValue={editingTransaction?.description || editingRecurring?.name || (activeAction === 'Transfer' ? 'Debit funding' : '')} placeholder={activeAction === 'Recurring' && recurringFlow === 'income' ? tr('Monthly salary', 'Sueldo mensual') : activeAction === 'Transfer' ? 'Debit funding' : tr('What was it for?', '¿Para qué fue?')} /></label>
                ...
                <button className="save-button" type="submit" disabled={saving}>{saving ? tr('Saving…', 'Guardando…') : editingTransaction || editingRecurring ? tr('Save changes', 'Guardar cambios') : `${tr('Save', 'Guardar')} ${activeAction.toLowerCase()}`}</button>
