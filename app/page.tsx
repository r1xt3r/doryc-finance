        ...
        <section className="hero-grid" id="overview" hidden={activeView !== 'overview'}>
          <article className="balance-card" data-reveal>
            <button className="balance-card-trigger" type="button" onClick={() => setShowBalanceDetail(true)} aria-haspopup="dialog">
              ...
              <div className="balance-bank-grid">{availableByBank.map(({ bank, balance }) => { const identity = bankIdentity(bank); return <span className={`bank-theme bank-${identity.key}`} key={bank}><i>{identity.mark}</i><small>{bank}</small><strong>{money(balance)}</strong></span>; })}</div>
        ...
        <section className="content-grid single-view" hidden={activeView !== 'accounts' && activeView !== 'payments'}>
          <article className="panel" id="accounts" data-reveal hidden={activeView !== 'accounts'}>
            ...
            <div className="account-list account-bank-list">{accountsByBank.map((group, bankIndex) => { const identity = bankIdentity(group.bank); return <section className={`account-bank-group bank-theme bank-${identity.key}`} key={group.bank}>
              <header className="account-bank-heading"><i className="bank-mark">{identity.mark}</i><span><small>{tr('BANK', 'BANCO')}</small><strong>{group.bank}</strong></span><span><strong>{money(group.balance)}</strong><small>{group.accounts.length} {tr(group.accounts.length === 1 ? 'account' : 'accounts', group.accounts.length === 1 ? 'cuenta' : 'cuentas')}</small></span></header>
