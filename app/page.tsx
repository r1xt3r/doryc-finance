        ...
        {activeAction && (
          <div className="modal-backdrop" role="presentation" onMouseDown={() => { if (!saving) { setActiveAction(null); setEditingTransaction(null); setEditingRecurring(null); } }}>
            <section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="modal-title" onMouseDown={(event) => event.stopPropagation()}>
              ...
              <form onSubmit={submitEntry} onChange={(event) => { const target = event.target as HTMLSelectElement; if (target.name === 'paymentMethod' && target.value === 'Cash' && cashAccounts[0]) { const source = event.currentTarget.elements.namedItem('fromAccountId') as HTMLSelectElement | null; if (source) source.value = cashAccounts[0].id; } }}>
