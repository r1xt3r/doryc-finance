...
export default function Home() {
  ...
  const accountsByBank = (() => {
    ...
    for (const account of data.accounts) grouped.set(account.bank, [...(grouped.get(account.bank) || []), account]);
          ...
          <section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="card-modal-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <form onSubmit={submitCardEntry}>
              {cardModal === 'sharedContribution' ? <><input type="hidden" name="recurringPaymentId" value={selectedCardId}/><input type="hidden" name="participantName" value={selectedSharedParticipant}/><input type="hidden" name="cycleMonth" value={currentCycleMonth}/><p className="loan-modal-summary">{selectedSharedParticipant} · {data.recurring.find((item) => item.id === selectedCardId)?.name}</p><label><span>{tr('Receive in account', 'Recibir en la cuenta')}</span><select name="accountId" required defaultValue=""><option value="" disabled>{tr('Select account', 'Selecciona una cuenta')}</option>{data.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><div className="field-row"><label><span>{tr('Amount received', 'Valor recibido')}</span><input name="amount" required autoFocus type="text" inputMode="decimal" defaultValue={data.recurring.find((item) => item.id === selectedCardId)?.sharedMembers.find((member) => member.name === selectedSharedParticipant)?.amount || ''}/></label><label><span>{tr('Date received', 'Fecha recibida')}</span><input name="date" required type="date" defaultValue={today}/></label></div></> : cardModal === 'account' ? <>
                <label><span>{tr('Account name', 'Nombre de la cuenta')}</span><input name="name" required autoFocus defaultValue={data.accounts.find((account) => account.id === selectedAccountId)?.name || ''} placeholder="Produbanco Savings" /></label>
                <label><span>{tr('Bank', 'Banco')}</span><select name="bank" required defaultValue={data.accounts.find((account) => account.id === selectedAccountId)?.bank || 'Pichincha'}><option>Pichincha</option><option>Produbanco</option><option>Pacifico</option><option>Guayaquil</option></select></label>
                <div className="field-row"><label><span>{tr('Account type', 'Tipo de cuenta')}</span><select name="accountType" required defaultValue={data.accounts.find((account) => account.id === selectedAccountId)?.accountType || 'Savings'}>{['Savings', 'Checking', 'Debit', 'Cash'].map((value) => <option key={value} value={value}>{accountTypeLabels[value]}</option>)}</select></label><label><span>{selectedAccountId ? tr('Current balance', 'Saldo actual') : tr('Starting balance', 'Saldo inicial')}</span><input name="balance" required type="text" inputMode="decimal" defaultValue={selectedAccountId ? data.accounts.find((account) => account.id === selectedAccountId)?.balance ?? 0 : 0} /></label></div>
              </> : cardModal === 'bankLoan' ? <>
