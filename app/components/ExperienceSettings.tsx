...
export default function ExperienceSettings({ value, language, onChange, open, onToggle }: { value: ExperiencePreferences; language: 'en' | 'es'; onChange: (value: ExperiencePreferences) => void; open: boolean; onToggle: () => void }) {
  ...
  const popover = open && typeof document !== 'undefined' ? createPortal(<section className="experience-popover experience-popover-portal" aria-label={es ? 'Preferencias de experiencia' : 'Experience preferences'} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}>
